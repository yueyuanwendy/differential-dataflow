...
fn main() {
    timely::execute_from_args(std::env::args().skip(2), move |worker| {
        ...
        let (mut input1, mut input2, mut input3, probe) = worker.dataflow::<_, _, _>(|scope| {
            ...
            let gis = data1
                .filter(|x: &Vec<String>| {
                    x[7].to_lowercase().contains("congenital") ||
                    x[12].to_lowercase().contains("congenital") ||
                     x[7].to_lowercase().contains("myasthenic") ||
                    x[12].to_lowercase().contains("myasthenic")
                })
...
//          
     ...
     //.inspect(move |x| class_wrt.write_record(&(x.0)).expect("Cannot write to file"));   
        ...
        let mut rdr = ReaderBuilder::new()
            ...
            .flexible(true)
            .delimiter(b'\t')
            .from_path("C:/Users/apple/Desktop/db/genemap2-161026-esc.txt").unwrap();
        ...
        let mut rdr = ReaderBuilder::new()
            ...
            .from_path("C:/Users/apple/Desktop/db/MUN0785.entire.csv").unwrap();
